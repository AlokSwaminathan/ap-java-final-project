...
class DrawingApp:
    def __init__(self, root):
        ...
        self.canvas = DrawingPad(
            self.root, settings.CANVAS_WIDTH, settings.CANVAS_HEIGHT)
    ...
    def add_toolbar_widgets(self):
        color_button = tk.Button(
            self.toolbar_frame, text="Color", command=self.choose_color)
        color_button.pack(pady=5)
        brush_size_slider = tk.Scale(self.toolbar_frame, from_=1, to=20,
                                     orient=tk.VERTICAL, label="Brush Size", command=self.change_brush_size)
